...
- js에서 this 는 dynamic 하게 binding 된다. this는 해당 라인이 어디서 처리(평가)되는가에 따라 달라질 수 있다는 말이다.
- callback(this.value) 처리(평가) 되는지 지점은 첫번째 프로미스의 resolve 함수내부이다.
- executor가 실행될 때 `executor(this.resolve.bind(this), this.reject.bind(this))` resolve 내부에서 this를 첫번째 프로미스로 binding 시켜 인자로 전달했다.
- bind에 의해 resolve 내부에서 this는 첫번째 프로미스를 참조하도록 고정(static) 된다.
...
- 핵심은 두번째로 생성되는 `resolve` 함수의 지연실행이다. 처음 작성했던 `callbakc`을 지연실행 했던 것을 참고하면 첫번째 프로미스의 `resolve` 내부에서 callback 대신에두번째로 생성되는 `Promise`의 `resolve`를 실행하게 만들면 된다. `callback` 이 아닌 두번째 `promise의 resolve`의 지연실행 인 것이다.
- 아래 코드를 보자.
...
- 그런면에서 this 의 동적 (dynamic) 스코프와 다른다. 위에서 this는 동적(dynamic)으로 결정된다고 했다. -`this.laterCalls.push(() => resolve(callback(this.value)));` 가 노출되어 있는 공간은 첫번째 프로미스의 메소드 `.then` 이하이다.
- 따라서 `.then`함수 스코프에서 this 는 첫번째 프로미스를 참조한다.
